...
public class ProductController {
	...
	public boolean modifyProduct(@RequestBody ProductsInfo info) {
		return service.modifyProduct(info);
	}
	
	@PostMapping(path = "/add-products")
	public boolean addProduct(@RequestBody ProductsInfo info) {
		return service.addProduct(info);
	}
	
	@GetMapping(path = "/get-product-by-id/{pid}")
	public ProductsInfo searchProductById(@PathVariable("pid") int pid) {
		return service.searchProductById(pid);
	}
	
	@PostMapping(path = "/add-to-cart")
	public boolean addToCart(@RequestBody ProductsInfo info) {
